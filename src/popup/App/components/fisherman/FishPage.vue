...
<template>
  <div>
    ...
    <div class="week-info">今天是星期{{ week }};<span v-if="isWorkDay">{{ chickenSoup }}</span>
      <el-button type="primary" size="mini" class="unitTypeBtn" @click="changeUnitType">切换</el-button>
    </div>
    <div class="border-wrap">
      ...
      <div v-if="Object.keys(weekend).length" class="row">
        距离周末还有<span> {{ weekend.days }} </span> <span v-if="unitType==1" class="unitType">次核酸</span>
        <span v-else class="unitType">天<span>
          {{ weekend.hours }} </span
      ...
      <div v-for="item in holidayList" :key="item.restDay[0]" class="row">
        ...
        > <span class="unitType" v-if="unitType==1">次核酸</span>
        <span  v-else class="unitType">
      ...
      <div class="row">
        距离发工资还有<span> {{ payOff }} </span> <span v-if="unitType==1" class="unitType">次核酸</span><span v-else class="unitType">天</span>
      </div>
...
export default {
  ...
  methods: {
    ...
    changeUnitType(){
      this.unitType==1?this.unitType=0:this.unitType=1
    }
  },
};
</script>
